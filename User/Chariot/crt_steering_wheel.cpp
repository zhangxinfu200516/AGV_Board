...
void Class_Steering_Wheel::CAN_RxAgvBoardCallback(Struct_CAN_Rx_Buffer *CAN_RxMessage)
{
    ...
    case 0x02A:
    {
        int16_t motion_feedback_troque;
        int16_t direct_feedback_troque;
        memcpy(&Power_Management.Motor_Data[0], CAN_RxMessage->Data, 2);
        memcpy(&motion_feedback_troque, CAN_RxMessage->Data + 2, 2);
        Power_Management.Motor_Data[0].feedback_torque = motion_feedback_troque/1000.0f;
        memcpy(&Power_Management.Motor_Data[1], CAN_RxMessage->Data + 4, 2);
        memcpy(&direct_feedback_troque, CAN_RxMessage->Data + 6, 2);
        Power_Management.Motor_Data[1].feedback_torque = direct_feedback_troque/1000.f;
    }
    break;
    case 0x02B:
    {
        int16_t motion_feedback_troque;
        int16_t direct_feedback_troque;
        memcpy(&Power_Management.Motor_Data[2], CAN_RxMessage->Data, 2);
        memcpy(&motion_feedback_troque, CAN_RxMessage->Data + 2, 2);
        Power_Management.Motor_Data[2].feedback_torque = motion_feedback_troque/1000.0f;
        memcpy(&Power_Management.Motor_Data[3], CAN_RxMessage->Data + 4, 2);
        memcpy(&direct_feedback_troque, CAN_RxMessage->Data + 6, 2);
        Power_Management.Motor_Data[3].feedback_torque = direct_feedback_troque/1000.f;
    }
    break;
    case 0x02C:
    {
        int16_t motion_feedback_troque;
        int16_t direct_feedback_troque;
        memcpy(&Power_Management.Motor_Data[4], CAN_RxMessage->Data, 2);
        memcpy(&motion_feedback_troque, CAN_RxMessage->Data + 2, 2);
        Power_Management.Motor_Data[4].feedback_torque = motion_feedback_troque/1000.0f;
        memcpy(&Power_Management.Motor_Data[5], CAN_RxMessage->Data + 4, 2);
        memcpy(&direct_feedback_troque, CAN_RxMessage->Data + 6, 2);
        Power_Management.Motor_Data[5].feedback_torque = direct_feedback_troque/1000.f;
    }
    break;
    case 0x02D:
    {
        int16_t motion_feedback_troque;
        int16_t direct_feedback_troque;
        memcpy(&Power_Management.Motor_Data[6], CAN_RxMessage->Data, 2);
        memcpy(&motion_feedback_troque, CAN_RxMessage->Data + 2, 2);
        Power_Management.Motor_Data[6].feedback_torque = motion_feedback_troque/1000.0f;
        memcpy(&Power_Management.Motor_Data[7], CAN_RxMessage->Data + 4, 2);
        memcpy(&direct_feedback_troque, CAN_RxMessage->Data + 6, 2);
        Power_Management.Motor_Data[7].feedback_torque = direct_feedback_troque/1000.f;
    }
    break;
    case 0x03A:
    {
...
{   //角度是以编码值为单位的（单圈编码值为8191），速度是以RPM为单位的
    ...
    {
        memcpy(&Power_Management.Max_Power, CAN_RxMessage->Data, 4);
	    memcpy(&Power_Management.Actual_Power,CAN_RxMessage->Data+4,4);
    }
}
